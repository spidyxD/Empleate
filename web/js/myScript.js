...
function uploadOfferer_CV(localeHX,localeHY) {
    if ($("#pdf").val().length < 1) {
        window.alert("NO CV ADJUNTO");
        return;
    }
    offerer = {nameOfferer: $("#nombre").val(), lastname: $("#apellido").val(), nationality: $("#nacionalidad").val(), email: $("#email").val(), phone: $("#telefono").val()
        , location_X: localeHX, location_Y: localeHY, residence: $("#direccion").val()};
    login = {username: $("#userO").val(), password: $("#keyO").val()};
    data = new FormData();
    data.append("offerer", JSON.stringify(offerer));
    data.append("login", JSON.stringify(login));
...
function addComp(localeHX,localeHY) {
    company = {nameCompany: $("#nombreEmpresa").val(), email: $("#website").val(), phone: $("#telefono").val(), description: $("#descripcion").val(), address: $("#direccion").val(), location_X: localeHX, location_Y: localeHY};
...
function validateComp(event) {
    ...
    $("#formularioComp").on("submit", validate);
}
...
function buildTableJobs(obj) {
    ...
    for (var i = 0; i < obj.length; i++) {
        console.log(obj[i].company.nameCompany);
        ...
        tr.html(
                ...
                "<td>" + obj[i].salary + "</td>" +
                "<td>" + obj[i].company.nameCompany + "</td>" +
                op);
        $("#results").append(tr);
    }
}